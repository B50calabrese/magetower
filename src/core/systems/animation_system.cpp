...
void AnimationSystem::process(Engine& engine, double delta_time_ms) {
  ...
  for (auto& entity : engine.getEntities()) {
    if ((this->getRequiredSignature() & entity->getSignature()) ==
      ...
      if (animation_component->getAnimationState() !=
      ...
      if (animation_component->getElapsedMs() >=
          animation_component->getDurationMs()) {
        // The animation is completed, time to clean up.
        position_component->setPosition(
            animation_component->getEndingPosition());
        ...
        position_component->setX(
            common::utils::tween(animation_component->getStartingPosition().x,
                                 animation_component->getEndingPosition().x,
                                 animation_component->getElapsedMs() /
                                     animation_component->getDurationMs()));

        position_component->setY(
            common::utils::tween(animation_component->getStartingPosition().y,
