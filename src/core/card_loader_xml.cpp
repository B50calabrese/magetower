...
namespace core {
    ...
    bool CardLoaderXML::loadCards(const std::string& file_path, CardRegistry& card_registry) {
        tinyxml2::XMLDocument doc;

        if (doc.LoadFile(file_path.c_str()) != tinyxml2::XML_SUCCESS) {
            common::utils::Logger::Error("XML Loader Error: Could not load card file: " + file_path + "\n");
            return false;
        }

        tinyxml2::XMLElement* root = doc.RootElement();
        if (!root || (std::string(root->Name()) != "Cards")) {
            common::utils::Logger::Error("XML Loader Error: Root element must be <Cards>\n");
            return false;
        }

        for (tinyxml2::XMLElement* card_element = root->FirstChildElement("Card");
            card_element != nullptr; card_element = card_element->NextSiblingElement("Card")) {
            const char* id = card_element->Attribute("Id");
            if (!id) {
                common::utils::Logger::Error("XML Loader Error: Card without id attribute\n");
                return false;
            }
            ...
            for (tinyxml2::XMLElement* component_element = card_element->FirstChildElement("Component");
            ...
            components.push_back(std::make_shared<SizeComponent>(core::CARD_DEFAULT_WIDTH, core::CARD_DEFAULT_HEIGHT));
            components.push_back(std::make_shared<PositionComponent>(glm::vec2(0.0f)));

            common::utils::Logger::Info("XML Loader: Registering card id: " + id_string + "\n");
    ...
    std::shared_ptr<Component> CardLoaderXML::parseComponent(tinyxml2::XMLElement* component_element) {
        ...
        if (type_string == "CardComponent") {
            ...
            const char* card_art = component_element->FirstChildElement("CardArt")->GetText();
            if (!name || !card_art) {
                common::utils::Logger::Error("XML Loader Error: CardComponent malformed\n");
                return nullptr;
            }
            std::string card_art_string = card_art;
            return std::make_shared<CardComponent>(
                common::resources::ResourceManager::LoadTexture(
                    ("assets/cards/art/" + card_art_string).c_str(), name, /*alpha=*/ true),
                name);
        }

        common::utils::Logger::Error("XML Loader Error: Unknown component type " + type_string + "\n");
        return nullptr;
    }
