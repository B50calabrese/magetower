...
namespace scenes {
    namespace battle {
        namespace systems {
            ...
            void EnemyDeckSystem::process(common::ecs::Engine& engine, double delta_time_ms) {

            }


            bool EnemyDeckSystem::handleEvent(common::ecs::Event& event, common::ecs::Engine& engine) {
                ...
                if (typeid(event) == typeid(EnemyDrawCardStartEvent)
                    ...
                    // If the enemy is starting to draw a card, initiate the animation and callback.
                    EnemyDeckSingletonComponent* deck_component = engine.getSingletonComponent<EnemyDeckSingletonComponent>();

                    // If the enym doesn't have enough cards, consume the event for now.
                    if (deck_component->getSizeOfDeck() <= 0)
                        return true;
                    ...
                    card->getComponent<AnimationComponent>()->setCallbackFunction(
                        [](common::ecs::Engine& engine, int entity_id) {
                            engine.getEntityFromId(entity_id)->addComponent<InEnemyHandTagComponent>();
