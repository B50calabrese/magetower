...
void CardHoldSystem::process(common::ecs::Engine& engine,
  ...
  for (auto& entity : engine.getEntities()) {
    if ((this->getRequiredSignature() & entity->getSignature()) ==
      ...
      MousePositionComponent* mouse_position =
          engine.getSingletonComponent<MousePositionComponent>();
      PositionComponent* position_component =
          entity->getComponent<PositionComponent>();
      position_component->setX(mouse_position->getX() -
                               core::CARD_HOLDING_HALF_WIDTH);
      position_component->setY(mouse_position->getY() -
                               core::CARD_HOLDING_HALF_HEIGHT);
    }
  }
}
  ...
  if (input_state_component->getInputState() ==
    ...
    for (auto& entity : engine.getEntities()) {
      if ((this->getRequiredSignature() & entity->getSignature()) ==
        ...
        if (common::utils::boundingBoxContains(core::BOARD_BOUNDING_BOX,
