...
const int PlayerHandSystem::SCALE_SPEED_WIDTH =
    (core::CARD_IN_HAND_ZOOM_WIDTH - core::CARD_IN_HAND_WIDTH) / 5;
const int PlayerHandSystem::SCALE_SPEED_HEIGHT =
    (core::CARD_IN_HAND_ZOOM_HEIGHT - core::CARD_IN_HAND_HEIGHT) / 5;
...
void PlayerHandSystem::process(Engine& engine, double delta_time_ms) {
  MousePositionComponent* mouse_position_component =
      engine.getSingletonComponent<MousePositionComponent>();

  bool needs_update = false;
  for (auto& entity : engine.getEntities()) {
    if ((this->getRequiredSignature() & entity->getSignature()) ==
        this->getRequiredSignature()) {
      // First grab the components we care about.
      SizeComponent* size_component = entity->getComponent<SizeComponent>();
      PositionComponent* position_component =
          entity->getComponent<PositionComponent>();

      if (mouseWithinSizePosition(*mouse_position_component,
                                  *position_component, *size_component)) {
        if (!entity->hasComponent<IsHoveredTagComponent>()) {
          entity->addComponent<IsHoveredTagComponent>();
          needs_update = true;
        } else if (size_component->getWidth() < core::CARD_IN_HAND_ZOOM_WIDTH ||
                   size_component->getHeight() <
        ...
        if (entity->hasComponent<IsHoveredTagComponent>()) {
          ...
          needs_update = true;
        } else if (size_component->getWidth() > core::CARD_IN_HAND_WIDTH ||
                   size_component->getHeight() > core::CARD_IN_HAND_HEIGHT) {
          needs_update = true;
        }
      }
    }
...
bool PlayerHandSystem::handleEvent(Event& event, Engine& engine) {
  if (typeid(event) == typeid(PlayerHandUpdateEvent)) {
    ...
    if (engine.getSingletonComponent<InputStateSingletonComponent>()
            ->getInputState() == InputStateSingletonComponent::FULL_CONTROL) {
...
bool PlayerHandSystem::handlePlayerClickedEvent(common::ecs::Engine& engine) {
  ...
  if (!input_state_component->getInputState() ==
      InputStateSingletonComponent::FULL_CONTROL) {
    return false;
  }

  MousePositionComponent* mouse_component =
      engine.getSingletonComponent<MousePositionComponent>();

  // Check that the player is clicking within the player's bounding box,
  // otherwise end early.
  if (mouseWithinBoundingBox(*mouse_component,
                             core::PLAYER_HAND_BOUNDING_BOX)) {
    for (auto& entity : engine.getEntities()) {
      if (entity->hasComponent<InPlayerHandTagComponent>()) {
        if (mouseWithinSizePosition(*mouse_component,
                                    *entity->getComponent<PositionComponent>(),
                                    *entity->getComponent<SizeComponent>())) {
          // The user is attempting to pick up card in their hand.
          input_state_component->setInputState(
              InputStateSingletonComponent::PLAYER_HOLDING_CARD);
