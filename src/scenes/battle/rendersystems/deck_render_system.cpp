...
DeckRenderSystem::DeckRenderSystem(
    std::shared_ptr<CardRenderUtil> card_render_util)
    : card_render_util(card_render_util) {}

void DeckRenderSystem::render(
    Engine& engine, std::shared_ptr<RendererManager> renderer_manager) {
  PlayerDeckSingletonComponent* player_deck =
      engine.getSingletonComponent<PlayerDeckSingletonComponent>();

  if (player_deck->getSizeOfDeck() > 0) {
    this->card_render_util->renderCardBack(core::PLAYER_DECK_POSITION,
                                           DeckRenderSystem::DECK_SIZE,
                                           renderer_manager);
  }

  EnemyDeckSingletonComponent* enemy_deck =
      engine.getSingletonComponent<EnemyDeckSingletonComponent>();

  if (enemy_deck->getSizeOfDeck() > 0) {
    this->card_render_util->renderCardBack(core::ENEMY_DECK_POSITION,
                                           DeckRenderSystem::DECK_SIZE,
                                           renderer_manager);
  }
}
