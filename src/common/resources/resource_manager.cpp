...
namespace common {
    namespace resources {
        ...
        Texture ResourceManager::LoadTexture(const char* file, std::string texture_name, bool alpha) {
            textures[texture_name] = loadTextureFromFile(file, alpha);
            return textures[texture_name];
        }

        Texture ResourceManager::LoadTextureRelative(const char* file, std::string texture_name, bool alpha) {
            return ResourceManager::LoadTexture(
                (ResourceManager::GetBaseFilePath() + file).data(), texture_name, alpha);
        }

        Shader ResourceManager::LoadShader(const char* vertexShaderFile, const char* fragmentShaderFile, std::string shader_name) {
            shaders[shader_name] = loadShaderFromFile(vertexShaderFile, fragmentShaderFile);
            return shaders[shader_name];
        }

        Shader ResourceManager::LoadShaderRelative(const char* vertexShaderFile, const char* fragmentShaderFile, std::string shader_name) {
